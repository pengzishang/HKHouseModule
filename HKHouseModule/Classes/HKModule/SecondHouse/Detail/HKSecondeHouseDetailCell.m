...
@interface HKSecondeHouseDetailCell()
/// 标题
...
/// 物业编号
@property (nonatomic, strong) UILabel  *serialNoLabel;
/// 更新日期
...
@implementation HKSecondeHouseDetailCell
...
- (void)setDetailModel:(HKSecondeHouseDetailModel *)detailModel {
...
- (void)setContent:(BOOL)isCNY {
    ...
    self.titleLabel.text = self.detailModel.houseName;
    [self.addressLabel rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text(self.detailModel.intSmDistrictName);
        confer.text(@" | ");
        confer.text(self.detailModel.buildingAddress);
    }];
    
    [self.serialNoLabel rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text(@"物业编号：");
        confer.text(rzFitStringEx(self.detailModel.serialNo, @"--"));
    }];
    
    [self.updateDateLabel rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text(@"更新日期：");
        confer.text(rzFitStringEx([self.detailModel.updateDate substringToIndex:10], @"--"));
    }];
    
    CGFloat price = [self.detailModel.price doubleValue];
    CGFloat transforprice = isCNY ? price * [rate doubleValue] : price;
    [self.priceLabel rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        if (isCNY) {
            confer.text([NSString stringWithFormat:@"%.2f万",(transforprice/10000.0)]).font(kFontBoldSize6(17)).textColor(HEX_RGB(0xFF0028));
        } else {
            confer.text([NSString stringWithFormat:@"%.2f萬",(transforprice/10000.0)]).font(kFontBoldSize6(17)).textColor(HEX_RGB(0xFF0028));
        }
        
        confer.text(@" ");
        confer.text(@"总价").font(kFontSize6(10)).textColor(HEX_RGB(0xBFB9AC));
    }];
    
    [self.flatLabel rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        if (isCNY) {
            confer.text([NSString stringWithFormat:@"¥%.2f元/㎡",(transforprice/[HKHouseUtil toSquare:[self.detailModel.area floatValue]])]).font(kFontBoldSize6(17)).textColor(HEX_RGB(0xFF0028));
        } else {
            confer.text([NSString stringWithFormat:@"HK$%.2f/呎",(transforprice/[self.detailModel.area floatValue])]).font(kFontBoldSize6(17)).textColor(HEX_RGB(0xFF0028));
        }
        
        confer.text(@" ");
        confer.text(@"单价").font(kFontSize6(10)).textColor(HEX_RGB(0xBFB9AC));
    }];
...
- (void)createUI {
    ...
    [self.contentView addSubview:self.serialNoLabel];
...
- (void)layout {
    ...
    [self.serialNoLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.updateDateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.serialNoLabel.mas_right).offset(20);
        make.top.equalTo(self.serialNoLabel);
    }];
    
    [self.priceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.titleLabel);
        make.top.equalTo(self.serialNoLabel.mas_bottom).offset(10);
    }];
...
- (UILabel *)serialNoLabel {
    if (!_serialNoLabel) {
        _serialNoLabel = [[UILabel alloc]init];
        _serialNoLabel.font = kFontSize6(13);
        _serialNoLabel.textColor = HEX_RGB(0x909399);
    }
...
- (UILabel *)updateDateLabel {
    if (!_updateDateLabel) {
        ...
        _updateDateLabel.font = kFontSize6(13);
        _updateDateLabel.textColor = HEX_RGB(0x909399);
    }
