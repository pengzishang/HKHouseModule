...
- (void)setupUI {
    ...
    [self.labelArray
     enumerateObjectsUsingBlock:^(UILabel* _Nonnull obj, NSUInteger idx,
        ...
        obj.textColor = self.configuration.ordinateTextColor;
        if (self.configuration.ordinateZoomThousand) {
            CGFloat num = (idx) * (self.top - self.bottom) / (self.labelArray.count - 1) + self.bottom;
            obj.text = [NSString stringWithFormat:@"%.0fK",num/1000.0];
        }else {
            obj.text = [NSString stringWithFormat:@"%.0f", (idx) * (self.top - self.bottom) / (self.labelArray.count - 1) + self.bottom];
            
        }
//        [numberFormatter stringFromNumber:@(num/1000)];
